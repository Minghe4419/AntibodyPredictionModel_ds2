...
Across demographic and clinical subgroups, consistent differences in antibody responses were observed. MARS predictions on `dat2` showed higher antibody levels in females (10.10 vs 9.83 for males), and lower levels in current smokers (9.80) compared to former and never smokers (9.99). Age and BMI also exhibited strong non-linear effects: responses peaked at moderate variable values and then declined as variable value increased. The marginal effect of each variable is the same as in dat1, reinforcing MARS model robustness. These biologically plausible patterns highlight the model’s ability to detect subgroup-specific variations.
...
Non-linear models outperformed linear ones in predicting antibody response. While LASSO served as a useful baseline, it missed complex trends. GAM captured smooth non-linear effects, but its test performance dropped, likely due to overfitting and time distribution mismatch. MARS showed the best balance of accuracy and interpretability, generalizing well across datasets. Key predictors included BMI, time since vaccination, gender, smoking status, and age, aligning with known biological patterns. These findings support using flexible models and highlight the importance of full time coverage for capturing antibody dynamics.
