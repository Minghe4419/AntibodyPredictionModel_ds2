...
```
Usin LOESS method, we observe linearity between predictors and the response. The plot shows that `bmi`, `time`, and `weight` has clear non linear trend against  resopnse `log_antibody`, indicating potential need to use GAM or non linear model.
...
vif_model <- lm(log_antibody ~ age + gender + smoking + height + weight + bmi + 
...
```{r}
# 找出所有因子变量中，只有一个水平的列名
drop_vars <- sapply(dat1, function(col) is.factor(col) && nlevels(col) < 2)
drop_names <- names(dat1)[drop_vars]

# 打印出来看一下（比如可能是 diabetes 或 hypertension）
print(drop_names)

# 创建模型矩阵时排除这些变量
x <- model.matrix(log_antibody ~ . - id - log_antibody - dataset - 
                    diabetes - hypertension, data = dat1)[, -1]
