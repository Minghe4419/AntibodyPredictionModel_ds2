...
dat1[categorical_name] <- lapply(dat1[categorical_name], factor)
...
```{r}
for (i in 1:(length(categorical_name)-1)) {
  for (j in (i+1):length(categorical_name)) {
    var1 <- categorical_name[i]
    var2 <- categorical_name[j]
...
y_pred_lasso <- predict(cv.lasso, newx = x_test, s = "lambda.1se")

rmse_lasso <- sqrt(mean((y_pred_lasso - y_test)^2))
print(paste("RMSE on test set (lambda.1se):", round(rmse_lasso, 4)))
```
...
print(paste("RMSE on test set (MARS):", round(mars_test_rmse, 4)))

gam.pred <- predict(gam.fit, newdata = dat2)
# test RMSE
gam_test_rmse = sqrt(mean((gam.pred - dat2[, "log_antibody"])^2))
print(paste("RMSE on test set (GAM):", round(gam_test_rmse, 4)))
```
